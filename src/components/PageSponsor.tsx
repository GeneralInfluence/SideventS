...
const PageSponsor: React.FC = () => {
  ...
  const totalAttendees = eventsForSelected.reduce((sum, ev) => {
    const count =
      typeof ev.attendees_shown === "number"
        ? ev.attendees_shown
        : parseInt(ev.attendees_shown) || 0;
    return sum + count;
  }, 0);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {selectedCategories.length > 0 ? totalAttendees : 0}
      </div>
      <h1
        style={{
          fontSize: 36,
            ...
            <div>
              ...
              {eventsForSelected.length === 0 ? (
                <div style={{ color: colors.accent }}>
                  No approved events in these categories.
                </div>
              ) : (
                eventsForSelected.map((ev) => {
                  ...
                  return (
                    <div key={eid} style={eventStyle}>
                      <strong>
                        {ev.event_name || ev.event_name || ev.Event}
                      </strong>
                      {ev.organizer && (
                        <span
                          style={{ color: colors.approved, fontWeight: 500 }}
                        >
                          {" "}
                          by {ev.organizer}
                        </span>
                      ...
                      <br />
                      {ev.venue && (
                        <span style={{ fontSize: 13 }}>{ev.venue}</span>
                      )}
                      {ev.full_address && (
                      ...
                      <span style={{ color: colors.accent, fontWeight: 600 }}>
                        Attendees:{" "}
                        {ev.attendees_shown !== undefined &&
                        ev.attendees_shown !== null
                          ? ev.attendees_shown
                          : "N/A"}
                      </span>
                    </div>
